                            ...
                            <div class="mb-3">
                                <div class="d-flex justify-content-between align-items-center mb-1">
                                    ...
                                    <div class="d-flex align-items-center gap-2">
                                        ...
                                        <form method="POST" action="{{ route('profile.skills.destroy', $skill) }}" class="d-inline" onsubmit="return confirm('Delete this skill?')">
                                            @csrf
                                            @method('DELETE')
                    ...
                    <div class="d-flex flex-wrap align-items-center">
                        @forelse($profile->interests as $interest)
                            <span class="badge-interest">
                                {{ $interest->name }}
                                <form method="POST" action="{{ route('profile.interests.destroy', $interest) }}" class="d-inline" onsubmit="return confirm('Remove interest?')">
                                    @csrf
                                    @method('DELETE')
        ...
        <div class="col-12 col-md-6 col-xl-3">
            <div class="hub-card d-flex flex-column justify-content-between">
                <div>
                    ...
                    @forelse($profile->studentProjects as $project)
                        <div class="project-item">
                            <div class="project-icon">
                                @if(str_contains(strtolower($project->title), 'hub') || str_contains(strtolower($project->title), 'collaborat'))
                                    <i class="bi bi-link-45deg"></i>
                                @elseif(str_contains(strtolower($project->title), 'library'))
                                    <i class="bi bi-journal-bookmark-fill text-warning"></i>
                                @elseif(str_contains(strtolower($project->title), 'ai') || str_contains(strtolower($project->title), 'chat'))
                            ...
                            <div class="flex-grow-1">
                                <div class="d-flex justify-content-between align-items-start">
                                    <span class="fw-bold text-dark small mb-0 d-block">{{ $project->title }}</span>
                                    <div>
                                        <button class="btn btn-sm btn-link text-secondary p-0 border-0 me-1" onclick="openEditProjectModal({{ $project->id }}, '{{ addslashes($project->title) }}', '{{ addslashes($project->description) }}', '{{ addslashes($project->technologies) }}')">
                                            <i class="bi bi-pencil"></i>
                                        </button>
                                        <form method="POST" action="{{ route('profile.projects.destroy', $project) }}" class="d-inline" onsubmit="return confirm('Delete project?')">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="btn btn-sm btn-link text-danger p-0 border-0">
                            ...
                            <div class="flex-grow-1 overflow-hidden">
                                <div class="d-flex justify-content-between align-items-center">
                                    ...
                                    <div>
                                        ...
                                        <form method="POST" action="{{ route('profile.portfolio-links.destroy', $link) }}" class="d-inline" onsubmit="return confirm('Delete link?')">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="btn btn-sm btn-link text-danger p-0 border-0">
                                                <i class="bi bi-trash"></i>
...
<div class="modal fade" id="addSkillModal" tabindex="-1">
    <div class="modal-dialog">
        <form method="POST" action="{{ route('profile.skills.store') }}" class="modal-content">
            @csrf
...
<form id="addInterestForm" method="POST" action="{{ route('profile.interests.store') }}" class="d-none">
    @csrf
...
<div class="modal fade" id="addProjectModal" tabindex="-1">
    <div class="modal-dialog">
        <form method="POST" action="{{ route('profile.projects.store') }}" class="modal-content">
            @csrf
            <div class="modal-header">
...
<div class="modal fade" id="addLinkModal" tabindex="-1">
    <div class="modal-dialog">
        <form method="POST" action="{{ route('profile.portfolio-links.store') }}" class="modal-content">
            @csrf
            <div class="modal-header">
...
<script>
    ...
    function openEditProjectModal(id, name, description, technologies) {
        document.getElementById('editProjectForm').action = '/profile/projects/' + id;
