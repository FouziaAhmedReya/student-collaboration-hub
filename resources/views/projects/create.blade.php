...
<div class="container-fluid px-lg-4">
    ...
    <form method="POST" action="{{ route('project-recruitments.store') }}">
        @csrf
        <div class="row g-4">
            ...
            <div class="col-12 col-lg-5">
                <div class="hub-card p-4 shadow-sm mb-4">
                    ...
                    <!-- Location Search with Nominatim Geocoding & Autocomplete -->
                    <div class="mb-3 position-relative">
                        <label class="form-label fw-semibold text-dark small">Search Place / Address</label>
                        <div class="input-group input-group-sm">
                            <input type="text" id="projectLocationSearchInput" class="form-control" placeholder="e.g. UB02 Campus, Mohakhali..." autocomplete="off">
                            <button type="button" class="btn btn-outline-primary" onclick="searchProjectPlace()">Search</button>
                        </div>
                        <div id="projectLocationSuggestions" class="list-group position-absolute w-100 shadow mt-1 border rounded-3 overflow-hidden" style="z-index: 1050; max-height: 220px; overflow-y: auto; display: none;"></div>
                    </div>
                    ...
                    <!-- Hidden Coordinates -->
                    <input type="hidden" id="latitude" name="latitude" value="{{ old('latitude', '23.7806') }}">
                    <input type="hidden" id="longitude" name="longitude" value="{{ old('longitude', '90.4068') }}">
                </div>

                <!-- Action Buttons -->
                <div class="d-flex align-items-center justify-content-end gap-2">
                    <a href="{{ route('project-recruitments.index') }}" class="btn btn-light border px-3">
                        Cancel
                    </a>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        HubMap.attachAutocomplete({
            inputId: 'projectLocationSearchInput',
            resultsContainerId: 'projectLocationSuggestions',
            onSelect: function (item, data) {
                ...
                if (projectPickerInstance) {
                    projectPickerInstance.setLocation(data.lat, data.lng);
                }
            }
        });
    });
    ...
    function searchProjectPlace() {
        ...
        HubMap.searchNominatim(query, function (results) {
            if (results && results.length > 0) {
                ...
                const placeName = first.name || (first.display_name ? first.display_name.split(',')[0].trim() : query);
                ...
                const container = document.getElementById('projectLocationSuggestions');
                if (container) {
